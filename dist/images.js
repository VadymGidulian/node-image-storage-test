...
class ImageStorage extends tiny_typed_emitter_1.TypedEmitter {
     ...
     */
    async getImagePath(id, size, { force = false } = {}) {
        let filePath = await (0, util_1.checkPath)(getImagePath(__classPrivateFieldGet(this, _ImageStorage_path, "f"), id, size));
        if (!filePath && force) {
            filePath = await (0, util_1.checkPath)(getImagePath(__classPrivateFieldGet(this, _ImageStorage_path, "f"), id));
        }
        return filePath;
    }
    /**
...
function getImagePath(root, id, size) {
    const [name, ext] = parseId(id);
    const hash = getHash(id);
    const fileName = [name, size, ext].filter(Boolean).join('.');
    return path.join(root, ...hash, fileName);
}
function parseId(id) {
    return id.split('.');
